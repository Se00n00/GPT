...
import dill
from lm_eval.utils import handle_non_serializable, setup_logging
...
class EvalModel(LM):
    ...
    def loglikelihood(
        ...
        def process_batch(batch_reqs, current_batch_size):
            ...
            try:
                ...
                for req in batch_reqs:
                    ...
                    full_enc, ctx_len, cont_enc = self._encode_and_slice(
                ...
                input_tensor = torch.tensor(
                    padded_inputs, dtype=torch.long, device=self.device
                )
                ...
                for i in range(len(batch_reqs)):
                    ...
                    seq_targets = torch.tensor(
                        batch_full_enc[i][start_pos + 1 : L_i],
                        dtype=torch.long,
                        device=self.device,
                    )
                ...
                if current_batch_size <= 1:
                    print(
                        "CUDA OOM even with batch_size=1! Falling back to CPU for this batch..."
                    )
                    ...
                    print(
                        f"CUDA Out of Memory. Reducing batch size from {current_batch_size} to {new_batch_size} and retrying..."
                    )
                    torch.cuda.empty_cache()
        ...
        idx = 0
        pbar = tqdm(
            total=len(requests), disable=disable_tqdm, desc="Evaluating loglikelihood"
        )
    ...
    def loglikelihood_rolling(
        ...
        results = []
        for req in tqdm(
            requests, disable=disable_tqdm, desc="Evaluating loglikelihood_rolling"
        ):
            try:
                text = req.args[0]
                ...
                for start_pos, chunk in chunks:
                    ...
                    targets = torch.tensor(
                        chunk[1:], dtype=torch.long, device=self.device
                    )
...
def get_tasks(task_type: Literal["Pretraining", "IFT", "PFT", "Reasoning"]):
    match task_type:
        case "Pretraining":
            return [
                "arc_challenge",
...
def main():
    parser = argparse.ArgumentParser(
        description="Evaluate custom GPT checkpoints using lm-eval."
    )
    parser.add_argument(
        "--checkpoint",
        type=str,
    ...
    parser.add_argument(
        "--tasks",
        type=str,
        default="hellaswag,arc_easy,arc_challenge,boolq,piqa,winogrande,mmlu",
        help="Comma-separated list of tasks to evaluate.",
    )
    parser.add_argument(
        "--tasks_type",
        type=str,
        choices=["default", "Pretraining", "IFT", "PFT", "Reasoning"],
        default="default",
        help="Type of the Task: Pretraining, IFT, PFT, Reasoning",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--tokenizer",
        type=str,
        default="Datasets/sft_tokenizer.json",
    ...
    if args.checkpoint is None:
        ...
        if not available_checkpoints:
            print(
                "Error: No checkpoints (.pt files) found in checkpoints/ or root directory."
            )
            sys.exit(1)
        ...
        print(
            f"Evaluating all {len(selected_checkpoints)} checkpoints: {selected_checkpoints}"
        )
    else:
    ...
    if args.tasks_type != "default":
        task_list = get_tasks(args.tasks)
    ...
    for cp_path in selected_checkpoints:
        ...
        try:
            ...
            if not remaining_tasks:
                print(
                    f"All requested tasks for {cp_name} are already evaluated! Skipping simple_evaluate."
                )
            ...
            if existing_results:
                ...
                existing_results["date"] = results.get(
                    "date", existing_results.get("date")
                )
            ...
            for task_name, task_results in results["results"].items():
                metrics = {
                    k: v for k, v in task_results.items() if not k.endswith("_stderr")
                }
                cp_metrics[task_name] = metrics
